...
public class IntermediateTest {
    ...
    public void testInfinite() {
        ...
        List<Integer> st = Stream.iterate(1, n -> n + 2).skip(2).limit(3).
                collect(Collectors.toList());
        assertEquals(Arrays.asList(5, 7, 9), st);
    }
    ...
    public void testFlatMap() {
        List<String> s = Arrays.asList("John Doe", "Jane Doe");
        List<String> z = Arrays.asList("Jack Doe", "Joe Doe");

        List<String> names = Stream.of(s, z).flatMap(l -> l.stream()).collect(Collectors.toList());
